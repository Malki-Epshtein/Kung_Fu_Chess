...
TEST_CASE("print(Board) - לוח ריק מודפס כנקודות בלבד") {
    Board board(3, 2);
...
TEST_CASE("print(Board) - כלי בודד מודפס בסימון הנכון") {
    Board board(3, 3);
...
TEST_CASE("print(Board) - דוגמת המסמך: לוח שלוש-על-שלוש עם כמה כלים") {
    Board board(3, 3);
...
TEST_CASE("print(Board) - כל סוגי הכלים משני הצבעים מודפסים באות הנכונה") {
    Board board(6, 2);
...
TEST_CASE("print(GameSnapshot) - מזהה זהה בפלט ל-print(Board) עבור אותו מצב") {
    Board board(3, 3);
    board.addPiece(make(1, Chess::Color::White, Chess::Kind::King, {0, 0}), {0, 0});
    board.addPiece(make(2, Chess::Color::Black, Chess::Kind::Rook, {0, 2}), {0, 2});

    std::ostringstream boardOut;
    BoardPrinter::print(board, boardOut);
