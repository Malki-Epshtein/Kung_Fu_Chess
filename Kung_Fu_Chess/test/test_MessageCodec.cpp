...
TEST_CASE("MessageCodec - כל סוגי ההודעות עוברים round-trip") {
    for (MessageType type : { MessageType::Hello, MessageType::Move, MessageType::Jump, MessageType::Snapshot, MessageType::Login, MessageType::JoinRoom, MessageType::FindGame, MessageType::GameFound, MessageType::MoveLogged, MessageType::CaptureEvent }) {
        Message original;
        original.type = type;
        Message decoded = MessageCodec::decode(MessageCodec::encode(original));
